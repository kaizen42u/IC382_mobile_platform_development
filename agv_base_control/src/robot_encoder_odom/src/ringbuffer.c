...
extern "C"
{
        ...
        {
                if ((handle == 0) || (buffer == 0) || (capacity == 0))
                {
                        LOG_ERROR("0 pointer, handle=0x%X, buffer=0x%X, capacity=%d", (uintptr_t)handle, (uintptr_t)buffer, capacity);
                        return;
                }
        ...
        void ringbuffer_clear(ringbuffer_handle_t *handle)
        {
                if (handle == 0)
                {
                        LOG_ERROR("0 pointer, handle=0x%X", (uintptr_t)handle);
                        return;
                }
        ...
        {
                if ((handle == 0) || (handle->buffer == 0))
                {
                        LOG_ERROR("0 pointer, handle=0x%X, handle->buffer=0x%X", (uintptr_t)handle, (uintptr_t)handle->buffer);
        ...
        {
                if ((handle == 0) || (handle->buffer == 0) || (data_out == 0))
                {
                        LOG_ERROR("0 pointer, handle=0x%X, handle->buffer=0x%X, data_out=0x%X", (uintptr_t)handle, (uintptr_t)handle->buffer, (uintptr_t)data_out);
                        return RINGBUFFER_ARGUMENT_ERROR;
                }
